import { ShowBusiness } from "../../src/business/ShowBusiness"
import { ShowDatabase } from "../../src/database/ShowDatabase"
import { BaseError } from "../../src/errors/BaseError"
import { IAddBookingInputDTO } from "../../src/models/Show"
import { AuthenticatorMock } from "../mocks/services/AuthenticatorMock"
import { HashManagerMock } from "../mocks/services/HashManagerMock"
import { IdGeneratorMock } from "../mocks/services/IdGeneratorMock"
import { ShowDatabaseMock } from "../mocks/ShowDatabaseMock"

describe("Testando ShowBusiness", () => {
    const showBusiness = new ShowBusiness(
        new ShowDatabaseMock() as unknown as ShowDatabase,
        new IdGeneratorMock(),
        new HashManagerMock(),
        new AuthenticatorMock()
    )

    test("reserva de ingresso bem sucedida", async () => {
        const input: IAddBookingInputDTO = {
            token: "token-mock",
            showId: "201",
        }

        const response = await showBusiness.addTicketBooking(input)

        expect(response.message).toEqual(
            "Pronto! Seu ingresso foi reservado com sucesso"
        )
    })

    test("retorna erro se o usuário não estiver logado", async () => {
        expect.assertions(2)

        try {
            const input: IAddBookingInputDTO = {
                token: "token-mock3",
                showId: "201",
            }

            await showBusiness.addTicketBooking(input)
        } catch (error: unknown) {
            if (error instanceof BaseError) {
                expect(error.statusCode).toEqual(401)
                expect(error.message).toEqual("Não autenticado")
            }
        }
    })

    test("retorna erro se show não for encontrado", async () => {
        expect.assertions(2)

        try {
            const input: IAddBookingInputDTO = {
                token: "token-mock",
                showId: "500",
            }

            await showBusiness.addTicketBooking(input)
        } catch (error: unknown) {
            if (error instanceof BaseError) {
                expect(error.statusCode).toEqual(404)
                expect(error.message).toEqual("Show não encontrado")
            }
        }
    })

    test("retorna erro se a mesma pessoa reservar ingresso para o mesmo show", async () => {
        expect.assertions(2)

        try {
            const input: IAddBookingInputDTO = {
                token: "token-astrodev",
                showId: "201",
            }

            await showBusiness.addTicketBooking(input)
        } catch (error: unknown) {
            if (error instanceof BaseError) {
                expect(error.statusCode).toEqual(409)
                expect(error.message).toEqual(
                    "Você já comprou este ingresso. Uma pessoa só pode reservar um ingresso por show"
                )
            }
        }
    })

    // test("retorna erro se os ingressos estiverem esgotados", async () => {
    //     expect.assertions(2)

    //     try {
    //         const input: IAddBookingInputDTO = {
    //             token: "token-astrodev",
    //             showId: "",
    //         }

    //         await showBusiness.addTicketBooking(input)
    //     } catch (error: unknown) {
    //         if (error instanceof BaseError) {
    //             expect(error.statusCode).toEqual(400)
    //             expect(error.message).toEqual("Ingressos esgotados")
    //         }
    //     }
    // })
})
