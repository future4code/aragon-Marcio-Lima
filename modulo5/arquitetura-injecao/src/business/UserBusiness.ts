...
export class UserBusiness {
    ...
    public deleteUser = async (input: IDeleteUserInputDTO) => {
        ...
        if (payload.id === idToDelete) {
            throw new Error("Não é possível deletar a própria contra")
        }
    ...
    public editUser = async (input: IEditUserInputDTO) => {
        ...
        if (name && name.length < 3) {
            throw new Error(
                "Parâmetro 'name' deve ter ao menos 3 caracteres"
            )
        }
        ...
        if (password && password.length < 6) {
            throw new Error(
                "Parâmetro 'password' deve ter ao menos 6 caracteres"
            )
        }
        ...
        if (payload.role === USER_ROLES.NORMAL) {
            if (payload.id !== idToEdit) {
                throw new Error(
                    "Usuários normais só podem editar a própria conta"
                )
            }
        }
        ...
        if (!userDB) {
            throw new Error("Conta a ser editada não encontrada")
        }
