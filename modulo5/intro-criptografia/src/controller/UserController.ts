...
export class UserController {
    ...
    public getAllUsers = async (req: Request, res: Response) => {
        ...
        try {
            ...
            const userDatabase = new UserDatabase()
            const isUserExists = await userDatabase.checkIfExistsById(
                payload.id
            )
        ...
        try {
            ...
            if (payload.id !== id) {
                if (payload.role !== USER_ROLES.ADMIN) {
                    errorCode = 403
                    throw new Error(
                        "Somente admins podem alterar os dados de outra conta"
                    )
                } else {
                    const userDatabase = new UserDatabase()
            ...
            if (
                ...
                return res.status(400).send("Email já existe no sistema")
            }
            res.status(errorCode).send({ message: error.message })
        }
    }
